...
// CheckPurlByNameType checks the all_urls table for the existence of entries matching a Purl Name and Type.
func (m *AllUrlsModel) CheckPurlByNameType(ctx context.Context, purlName string, purlType string) (bool, error) {
	s := ctxzap.Extract(ctx).Sugar()
	if len(purlName) == 0 {
		s.Error("Please specify a valid Purl Name to query")
		return false, errors.New("please specify a valid Purl Name to query")
	}
	if len(purlType) == 0 {
		s.Error("Please specify a valid Purl Type to query")
		return false, errors.New("please specify a valid Purl Type to query")
	}
	var exists bool
	...
	if err != nil {
		...
		return false, fmt.Errorf("failed to query the all_urls table: %v", err)
	}
	return exists, nil
}
